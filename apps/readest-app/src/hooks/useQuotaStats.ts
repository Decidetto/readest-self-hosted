...
import { getDeploymentMode } from '@/services/runtimeConfig';
  ...
  useEffect(() => {
    ...
    const storageQuota: QuotaType = {
      name: briefName
        ? _('Storage')
        : selfHosted
          ? _('Server Storage Limit')
          : _('Cloud Sync Storage'),
      tooltip: selfHosted
        ? _('{{percentage}}% of the server storage limit used.', {
    ...
    const translationQuota: QuotaType = {
      name: briefName
        ? _('Translation')
        : selfHosted
          ? _('Daily Translation Limit')
          : _('Translation Characters'),
      tooltip: selfHosted
        ? _('{{percentage}}% of the daily server translation limit used.', {
